...
onready var Explosion = load("res://Enemy/Bullet.tscn")
...
func damage(d):
	health -= d 
	if health <= 0: 
		Effects = get_node_or_null("/roots/Game/Effects")
		if Effects != null: 
...
func _on_Timer_timeout():
	var Player = get_node_or_null("/root/Game/Player_ContainerPlayer")
